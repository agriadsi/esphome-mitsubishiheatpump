// begin(bitrate, SERIAL_8E1, rx, tx)
// write
// available
// read
...
#include "HardwareSerialMinIDF.h"
...
//                log_e("Bad UART Number");
    ...
    _uart = uartBegin(_uart_nr, (baud > 0) ? baud : 9600, config, rxPin, txPin, _rxBufferSize, _txBufferSize, invert, rxfifo_full_thrhd);
/*
    if (!baud) {
...
{
    return uartReadBytes(_uart, buffer, length, 1000);
}
